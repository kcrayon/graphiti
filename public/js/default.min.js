...
return this},buildURL:function(){var h=this.urlBase,f=[],b=Graphiti.timeFrameOptions(),b=$.extend({},this.options,b);$.each(b,function(b,h){f.push(b+"="+h)});$.each(this.parsedTargets,function(b,h){f.push("target="+h)});f.push("_timestamp_="+(new Date).getTime());return h+f.join("&")+"#.png"},image:function(h){this.updateOptions(h.dimensions());h.bind("load",function(){$(this).removeClass("loading")}).addClass("loading").attr("src",this.buildURL());return h},toJSON:function(){return JSON.stringify({options:this.options,
targets:this.targets},null,2)},save:function(h,f){var b,l={graph:{title:this.options.title||"Untitled",url:this.buildURL(),json:this.toJSON()}};$.isFunction(h)&&(f=h,h=null);h?(b="/graphs/"+h,l._method="PUT"):b="/graphs";$.ajax({url:b,data:l,type:"post",success:f})},snapshot:function(h,f){$.ajax({type:"post",dataType:"json",url:"/graphs/"+h+"/snapshot",success:function(b){f(b.url)}})}};
